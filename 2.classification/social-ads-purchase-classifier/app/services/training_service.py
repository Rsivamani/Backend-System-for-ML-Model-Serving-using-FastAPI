"""
Training Service — Social Network Ads Purchase Prediction (DT-CLF base)
...
Selection metric: F1 Score (handles class imbalance better than raw accuracy)
"""
...
CANDIDATE_MODELS = {
    ...
    "DecisionTreeClassifier": DecisionTreeClassifier(
        criterion="entropy", max_depth=settings.max_depth, random_state=settings.random_state
    ),
    "RandomForestClassifier": RandomForestClassifier(
        n_estimators=100, criterion="entropy", random_state=settings.random_state
    ),
...
class TrainingService:
    """
    Trains all candidate classifiers on Social_Network_Ads dataset,
    compares Accuracy/Precision/Recall/F1, detects overfitting,
    and saves only the best-performing pipeline.
    """
    ...
    def train(self) -> Dict[str, Any]:
        ...
        for name, estimator in CANDIDATE_MODELS.items():
            ...
            logger.info(
                "%s | acc=%.4f | F1=%.4f | overfit_gap=%.4f",
                name, acc, f1, overfit_gap,
            )

            # Select by F1 score — robust for imbalanced classes
            if f1 > best_f1:
        ...
        return {
            ...
            "selection_reason": (
                ...
                "indicating balanced precision-recall performance on the hold-out set."
            ),
            "note": "Model selection is data-dependent. The chosen model performed best on this dataset and may vary with different data.",
    ...
    def _load_data(self) -> pd.DataFrame:
        ...
        logger.info("Loaded %d rows from %s", len(df), RAW_DATA_PATH)
        return df
